...
/// 키캡 소리. 음원 파일을 굽지 않고 파형을 그 자리에서 만든다.
///
/// 스위치 소리는 한 방이 아니라 몇 ms 간격으로 이어지는 타격 여러 개다. 청축을 예로 들면
...
/// `--click` 으로 전부 한 번씩 들어 보면서 숫자를 맞추면 된다.
enum Sound {
    ...
    static func down() { if let p = current { play(wave(p.id + ".down", p.down)) } }
    static func up() { if let p = current { play(wave(p.id + ".up", p.up)) } }

    /// 설정과 무관하게 한 벌을 들려준다 — 두 방을 한 노드에 줄 세워 톡탁 하고 이어 난다.
    /// 메뉴에서 고르는 즉시 부르는 자리라 재우지 않는다(메뉴가 멈춘다).
    static func preview(_ p: Profile) {
        play(wave(p.id + ".down", p.down), wave(p.id + ".up", p.up))
    }
    ...
    /// 소리가 점점 뒤로 밀린다(한 벌이 66~126ms 라 초당 10번쯤부터 쌓인다). 돌려 쓰면 겹쳐서 나고,
    ...
    private static func play(_ waves: AVAudioPCMBuffer...) {
        ...
        // 3 초면 아무리 늦은 소리도 다 나간 뒤다(가장 긴 한 방이 84ms).
        idle?.invalidate()
