...
| 프리셋 | 구조 |
|---|---|
| 청축 — 찰칵찰칵 | 기본값. 밝고 짧은 클릭 재킷 + 5ms 뒤 바닥 타격. 뗄 때 재킷이 한 번 더 튄다 |
| 갈축 — 도각도각 | 클릭 재킷 없이 옅은 걸림 소리 + 바닥 타격이 주인공 |
| 적축 — 툭툭 | 리니어. 걸림도 클릭도 없이 바닥과 천장에 닿는 소리만 |

음원 파일은 들어 있지 않다. `Sound.swift` 가 실행 중에 파형을 굽는다(44.1kHz 모노, 한 방이
30~96ms). 스위치 소리는 한 방이 아니라 몇 ms 간격으로 이어지는 타격 여러 개라서, `Voice` 는
시작 시각이 다른 `Hit` 의 묶음이다 — 청축이 "찰칵찰칵" 인 건 누를 때 클릭 재킷(3~7kHz의 아주
짧고 밝은 소리)과 바닥 타격(저역)이 5ms 차이로 겹치고, 뗄 때 재킷이 제자리로 돌아오며 한 번 더
울리기 때문이다. 실제로 기본 프리셋의 첫 4ms 는 에너지의 97%가 3kHz 위에, 이어지는 5~20ms 는
96%가 800Hz 아래에 있다.

번들 크기가 안 늘고, 소리를 하나 더 넣는 것도 `Sound.profiles` 에 한 벌이다 — 우클릭 메뉴는 이
표를 그대로 따라간다. 파형은 `--click` 으로 들어 보며 맞춘다.
