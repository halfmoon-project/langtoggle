...
- **우하단 화살표에서 드래그** — 키캡 크기를 28pt에서 88pt 사이로 바꾼다. 키캡 우하단 모서리에
  마우스를 올리면 크기 조절 화살표가 나타나고, 끄는 방향을 그대로 따라간다. 그냥 클릭하면 평소대로 언어가 전환된다.
  우클릭 메뉴의 **크기** 에 프리셋도 있다. 크기는 자동 저장된다.
- **우클릭** — 로그인 시 자동 실행, 크기, 아이콘 새로고침, 종료 메뉴를 연다.
